...
// 9DOF Razor IMU SEN-10125 using HMC5843 magnetometer
    ...
    magnetom[0] = -1 * (int16_t)((((int) buff[2]) << 8) | buff[3]);  // X axis (internal sensor -y axis)
    magnetom[1] = -1 * (int16_t)((((int) buff[0]) << 8) | buff[1]);  // Y axis (internal sensor -x axis)
    magnetom[2] = -1 * (int16_t)((((int) buff[4]) << 8) | buff[5]);  // Z axis (internal sensor -z axis)
// 9DOF Razor IMU SEN-10736 using HMC5883L magnetometer
#elif HW__VERSION_CODE == 10736
    // MSB byte first, then LSB; Y and Z reversed: X, Z, Y
    magnetom[0] = -1 * (int16_t)((((int) buff[4]) << 8) | buff[5]);  // X axis (internal sensor -y axis)
    magnetom[1] = -1 * (int16_t)((((int) buff[0]) << 8) | buff[1]);  // Y axis (internal sensor -x axis)
...
// 9DOF Sensor Stick SEN-10183 and SEN-10321 using HMC5843 magnetometer
    ...
    magnetom[1] = -1 * (int16_t)((((int) buff[2]) << 8) | buff[3]);  // Y axis (internal sensor -y axis)
    magnetom[2] = -1 * (int16_t)((((int) buff[4]) << 8) | buff[5]);  // Z axis (internal sensor -z axis)
